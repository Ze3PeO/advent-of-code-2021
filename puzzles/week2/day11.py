    ...
    for step in range(100):
        ...
        for y in range(FIELD_DIMEN):
            for x in range(FIELD_DIMEN):
                increase(field, x, y, has_flashed)
...
def increase(field, x, y, has_flashed):
    if not has_flashed[y][x]:
        field[y][x] += 1

    if field[y][x] > 9:
        flash(field, x, y, has_flashed)


def flash(field, x, y, has_flashed):
    if has_flashed[y][x]:
        return
    ...
    for j in range(y - 1, y + 2):
        for i in range(x - 1, x + 2):
            if is_within_bounds(i, j) and (j != y or i != x):
                increase(field, i, j, has_flashed)
